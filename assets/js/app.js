document.addEventListener("DOMContentLoaded", () => {
  ...
  if (servicesCards.length != 0) {
    function enableServicesSliders() {
      document.querySelectorAll(".services__cards").forEach((cards) => {
        if (
          !cards.classList.contains("swiper-initialized") &&
          !cards.classList.contains("swiper")
        ) {
    ...
    function disableServicesSliders() {
      servicesSliders.forEach(({ slider, cards }) => {
        ...
        cards
          ...
          .forEach((card) => card.classList.remove("swiper-slide"));
      });
      servicesSliders = [];
    }
  ...
  const widget = document.querySelector(".rev-widget__item");
  const showWidgetBtns = document.querySelectorAll(".show-widget");
  const swiperContainer = document.querySelector(".rev-widget__swiper");

  const widgetSlider = new Swiper(".rev-widget__swiper", {
    slidesPerView: 1,
    spaceBetween: 15,
    navigation: {
      prevEl: ".rev-widget__nav .btn-prev",
      nextEl: ".rev-widget__nav .btn-next",
    },
    on: {
  ...
  function playCurrentSlideVideo() {
    ...
    if (activeSlide) {
      activeSlide.play().catch((e) => {
        console.warn("Не удалось запустить видео:", e);
      });
    }
  }

  showWidgetBtns.forEach((btn) => {
